...
public class SchemeFactory 
{
	...
	public ExternalStorageInterface createStorage(String storage, boolean reset)
	{
		...
		case "MongoDB": s = new MongoDBStorage(reset); break;
		case "MySQL": s = new MySQLStorage(reset); break;
		case "SFTP": s = new SFTPStorage(reset); break;
		default: 
	...
	{		
		...
		switch(clientDesc[0])                                     // SCHEME SWITCH !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		{
		case "RAM": 
		...
		case "ODStree": 
			...
			c = new ODStreeClient(); 
			break;
		case "ODStree_Caching": 
			assert(clientDesc.length == 1);
			c = new ODStreeCachingClient(); 
			break;
		case "TinyOAT": 
			assert(clientDesc.length == 1);
			c = new TinyOATClient(); 
			break;
		case "TinyOAT_Caching": 
			assert(clientDesc.length == 1);
			c = new TinyOATcachingClient(); 
			break;
		case "ObliviousMatrix": 
			assert(clientDesc.length == 1);
			c = new ObliviousMatrixClient(); 
			break;
		case "ObliviStore":
