...
{
	public enum RequestType { GET, PUT, GETROW, GETCOL, PUTROW, PUTCOL, GETALL };
	...
	public static Request fromString(String fromString)
	{
		...
		case "GETALL": return new GetAllRequest(Long.parseLong(s[1]), s[2]);
		...
		case "GETROW": return new GetRowRequest(Long.parseLong(s[1]), s[2]);
			
		case "GETCOL": return new GetColRequest(Long.parseLong(s[1]), s[2]);
		
		case "PUTROW": if(s.length < 4) { Errors.error("Invalid fromString (PUTROW): " + fromString);	}
			DataItem di4 = null; String di4FromString = s[3];
			if(di4FromString.startsWith(InflatableDataItem.stringPrefix)) { di4 = new InflatableDataItem(di4FromString); } 
			else { di4 = new SimpleDataItem(di4FromString); }
			return new PutRowRequest(Long.parseLong(s[1]), s[2], di4);
				
		case "PUTCOL": if(s.length < 4) { Errors.error("Invalid fromString (PUTROW): " + fromString);	}
			DataItem di5 = null; String di5FromString = s[3];
			if(di5FromString.startsWith(InflatableDataItem.stringPrefix)) { di5 = new InflatableDataItem(di5FromString); } 
			else { di5 = new SimpleDataItem(di5FromString); }
			return new PutColRequest(Long.parseLong(s[1]), s[2], di5);
