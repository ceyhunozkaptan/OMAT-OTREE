...
import eoram.cloudexp.evaluation.*;
...
//	private static final String defSessionFileArg = "C:\\Users\\Workstation\\Documents\\workspace_java\\curious\\state\\session.state"; //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	private static final String defRSFileArg = "./"; //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
//	private static final String defRSFileArg = "C:\\Users\\Workstation\\Documents\\workspace_java\\curious\\"; //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	private static final String defRunnerArg = runnerArgValSerial;
...
//	private static final String defPerfSummaryFPArg = "C:\\Users\\Workstation\\Documents\\workspace_java\\curious\\summary.perf"; //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	{	
		...
		for (int power = 10; power < 11; power = power+1 )
		{
...
//					"0debcc1c9e",
			...
			for(int expNo = 0; expNo < keys.length && resume; expNo++)
			{
				System.gc();
				...
				String sessionFilePath = "./state/session.state";
				
				System.out.println("Session File Path" + sessionFilePath);
...
//					System.out.println("Enter Experiment Hash:");
//					String key1 = keyboard.next();
					
					String key = keys[expNo];
					
					sessionFilePath = "./state/" + key + ".session.state";
				...
				{			
					...
					String client = getNonOptionalArgs(args).get(1);
					if (client.equalsIgnoreCase("TinyOAT") || client.equalsIgnoreCase("TinyOAT_Caching") || client.equalsIgnoreCase("ODStree") || client.equalsIgnoreCase("ODStree_Caching") )
						ss.rsFilePath = getArgValue(args, rsFileArg, defRSFileArg + "input (tree).rs.csv");
					else if(client.equalsIgnoreCase("ObliviousMatrix"))
						ss.rsFilePath = getArgValue(args, rsFileArg, defRSFileArg + "input (matrix).rs.csv");
					else
						ss.rsFilePath = getArgValue(args, rsFileArg, defRSFileArg + "input (default).rs.csv");
	...
	//			final File stateFile = new File("C:\\Users\\Workstation\\Documents\\workspace_java\\curious\\state\\" + experimentHash + ".state");
				...
				if(generateRS == true)
				{
					...
					System.out.println("Generating random request sequence of length " + generateLength + " (init: " + initLength + ")...");
				...
				{ 
					if(ss.extracted == true)
					{
						...
						System.out.println("Reading extracted trace from " + ss.rsFilePath);
					...
					else
					{
						...
						System.out.println("Reading requests from " + ss.rsFilePath);
				...
				{			
					...
					{				
						...
						System.out.println("Starting experiments [runner: " + runnerStr + "] " + desc + "; will process " + length + " next requests.");
	...
	//					System.gc();
						...
						System.out.println(reqsProcessed + " requests processed.");
						...
						final File logFile = new File("./log/" + client + "_" + storage + "_" + clientParams.maxBlocks + "_" + schemeStr + ".perfeval.log");
						
	//					final File matFile = new File("./log/" + client + "_" + storage + "_" + clientParams.maxBlocks + "_" + schemeStr + ".perfeval.txt");
						final File matFile = new File("./log/oat_experiment.txt");
						...
						pelog.dumpToFile(logFile,matFile);
				...
				sessionFilePath = "./state/" + experimentHash + ".session.state";
				sessionFile = new File(sessionFilePath);
	...
	private static void initClientParams(String[] args, int power)
	{				
		ClientParameters clientParams = ClientParameters.getInstance();
...
//		clientParams.maxBlocks = getArgLongValue(args, maxBlocksArg, defMaxBlocks);
		clientParams.maxBlocks = (1 << power) - 1;
		
		String client = getNonOptionalArgs(args).get(1);
		System.out.println(client);
		if (client.equalsIgnoreCase("TinyOAT") || client.equalsIgnoreCase("TinyOAT_Caching") || client.equalsIgnoreCase("ODStree") || client.equalsIgnoreCase("ODStree_Caching") ){
			clientParams.contentByteSize = getArgIntValue(args, blockByteSizeArg, defBlockByteSize) - encryptionOverhead - 16;
//			clientParams.contentByteSize = asd - encryptionOverhead - 16;
		}
			
		else if(client.equalsIgnoreCase("ObliviousMatrix")){
			clientParams.contentByteSize = getArgIntValue(args, blockByteSizeArg, defBlockByteSize) - encryptionOverhead - 8;
//			clientParams.contentByteSize = asd - encryptionOverhead - 8;
		}
			
		else{
			clientParams.contentByteSize = getArgIntValue(args, blockByteSizeArg, defBlockByteSize) - encryptionOverhead;
//			clientParams.contentByteSize = asd - encryptionOverhead;
		}
