...
import eoram.cloudexp.schemes.TinyOATcaching.Tree.Bucket;
...
//			System.out.println(((int) (Math.pow(2,((D+1)/2))-1)) + "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" + cache.size());
...
//					System.out.println("[PathORAMBasic (BuildTree)] (R" + recLevel +") putting block " + id + " to label " + label + " (objectKey: " + recLevel + "#" + (treeData.indexOf(i)) + ").");
				...
				if (treeData.indexOf(i) < (int) (Math.pow(2,((D+1)/2))-1)){
					long start = System.nanoTime();					
					cache.set((treeData.indexOf(i)), temp);
...
//					v.add(sop);
		...
		private Bucket[] getBucketsFromPath(long reqId, int leaf, int level) 
		{
			...
			for (int i = 0; i < ret.length; i++) 
			{
				...
				if (path[path.length-1-i] < (int) (Math.pow(2,((D+1)/2))-1)){
					long start = System.nanoTime();				
//					ret[k] = cache.get(path[path.length-1-i]);
					ret[k] = new Bucket(new Block());
					k++;
					long end = System.nanoTime();
					System.out.println("[PO (getBucketsFromPath)] Read from Cache => objectKey: " + objectKey + " in " + (end - start) / 1000000.0 + " ms");
				}
				else{									
		...
		class Block { 
			...
			public Block() {
				...
				id = -1; // id == N marks a dummy block, so the range of id is from 0 to N, both ends inclusive. Hence the bit length of id is D+1.
				treeLabel = -1;
				leftId = -1;
				rightId = -1;
				leftLabel = -1;
	...
	public TinyOATcaching (SecureRandom rand) 
	{
		rnd = rand;
...
//				current = blk.rightId;
				if (current != -1){
					n = rand.nextInt(tr.N);
					start = System.nanoTime();
					blocks = tr.readBuckets(reqID, n + tr.N-1, curLevel);
...
//					blocks = tr.readBuckets(reqID, blk.rightLabel + tr.N-1, curLevel);
		...
		System.out.println(downTime+decTime);
		System.out.println("==============================================================================");
		System.out.println("");
...
//						cache.set(objKey, bucket);
		...
		try {
			...
			bw.write((int) ((upTime) / 1000000.0)+"," );
			bw.write((int) ((upTime+encTime+downTime+decTime) / 1000000.0)+"," );
			bw.newLine();
			bw.flush();
			bw.close();
		...
		System.out.println(upTime+encTime);
		System.out.println("==============================================================================");
		System.out.println("");
